...
void send_with_response(uint8_t* message_out, uint16_t message_out_length,
    uint16_t expected_response_length, void (*callback)(void*, uint8_t*, uint16_t)) {
...
void process_messages(void* param, uint16_t connection_handle, uint16_t attribute_handle) {
    ...
    if(m_expected_response_length > 0 && m_response_message_progress == m_expected_response_length) {
        void (*response_callback)(void*, uint8_t*, uint16_t) = m_response_callback;
        m_response_callback = NULL;
        m_response_message_progress = 0;
        m_expected_response_length = 0;
        response_callback(param, m_response_message_buffer, m_expected_response_length);
    }
